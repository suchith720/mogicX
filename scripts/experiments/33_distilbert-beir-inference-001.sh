...
do
	...
	CUDA_VISIBLE_DEVICES=0,1 python mogicX/33_distilbert-beir-inference-001.py --dataset $dataset --prediction_suffix $suffix --save_test_prediction \
		--do_train_inference --save_train_prediction >> $output_file

	# if [ $dataset == "msmarco" ]
	# then
	# 	CUDA_VISIBLE_DEVICES=2,3 python mogicX/33_distilbert-beir-inference-001.py --dataset $dataset --prediction_suffix $suffix \
	# 		--save_test_prediction >> $output_file
	# else
	# 	CUDA_VISIBLE_DEVICES=2,3 python mogicX/33_distilbert-beir-inference-001.py --dataset $dataset --prediction_suffix $suffix \
	# 		--save_test_prediction --only_test >> $output_file
	# fi
done
