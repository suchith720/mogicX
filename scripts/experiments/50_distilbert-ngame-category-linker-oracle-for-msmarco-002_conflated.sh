...
datasets="fever fiqa nq cqadupstack/android cqadupstack/english cqadupstack/gaming cqadupstack/gis cqadupstack/mathematica cqadupstack/physics \
...
do
	...
	if [ $dataset == "msmarco" ]
	then
		CUDA_VISIBLE_DEVICES=0,1,2,3 python mogicX/50_distilbert-ngame-category-linker-oracle-for-msmarco-mteb-inference.py --dataset $dataset --expt_no 0 --save_test_prediction --prediction_suffix $suffix >> $output_file
	else
		CUDA_VISIBLE_DEVICES=0,1,2,3 python mogicX/50_distilbert-ngame-category-linker-oracle-for-msmarco-mteb-inference.py --dataset $dataset --expt_no 0 --save_test_prediction --prediction_suffix $suffix --only_test >> $output_file
	fi
