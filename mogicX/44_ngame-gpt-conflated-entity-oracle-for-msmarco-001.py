...
if __name__ == '__main__':
    ...
    def model_fn(mname):
        model = DBT024.from_pretrained(mname, margin=0.3, tau=0.1, n_negatives=10, apply_softmax=True, normalize=True, use_encoder_parallel=True)
        return model
    
    def init_fn(model): 
        model.init_dr_head()

    metric = PrecReclMrr(block.test.dset.n_lbl, block.test.data_lbl_filterer, pk=10, rk=200, rep_pk=[1, 3, 5, 10], rep_rk=[10, 100, 200], mk=[5, 10, 20])
